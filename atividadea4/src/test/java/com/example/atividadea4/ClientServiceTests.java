...
public class ClientServiceTests {
    ...
    void setUp() throws Exception {
        ...
        Mockito.doThrow(EmptyResultDataAccessException.class).when(repo).deleteById(idNaoExistente);
        Mockito.doThrow(DataIntegrityViolationException.class).when(repo).deleteById(idDependente);
    }
    ...
    public void apagarDeveLancarUmaExcecaoQuandoOIdNaoExiste() {
        ...
        Mockito.verify(repo, Mockito.times(1)).deleteById(idNaoExistente);
    }
    ...
    public void retornarUmaPaginaComTodosOsClientes() {
        ...
        Mockito.verify(repo, Mockito.times(1)).findAll(pageRequest);
    }
    ...
    public void retornarUmaListaComClientesComRendaMaiorQue() {
        ...
        // assign
        Mockito.when(repo.findByIncomeGreaterThan(renda)).thenReturn(List.of(new Client()));
    ...
    public void retornarOClientePeloId() {
        ...
        // act
        Mockito.when(repo.findById(id)).thenReturn(Optional.of(new Client()));
    ...
    public void atualizarDeveRetornarClienteAtualizado() {
        ...
        Client clienteExistente = new Client(id,"Conceição Evaristo", "10619244881", 1500.0, Instant.ofEpochSecond(0), 2);
        ClientDTO dtoAtualizado = new ClientDTO(id, "Pedro Augusto", "12345678901", 1900.0, instantFromString, 3);
        

        // act
        Mockito.when(repo.getOne(id)).thenReturn(clienteExistente);
        Mockito.when(repo.save(Mockito.any(Client.class))).thenReturn(clienteExistente);
    ...
    public void atualizarDeveRetornarExceçãoQuandoIdNaoExistir() {
        ...
        Mockito.when(repo.getOne(idInvalido)).thenThrow(ResourceNotFoundException.class);
        ...
        Mockito.verify(repo).getOne(idInvalido);
    }
    ...
    public void retornarUmClientDTONaInsercaoDeUmNovoCliente() {
        ...
        Client entidadeSalva = new Client(id, "João Silva", "12345678901", 2500.0, birthDate, 1);

        Mockito.when(repo.save(Mockito.any(Client.class))).thenReturn(entidadeSalva);
        ...
        Mockito.verify(repo).save(Mockito.any(Client.class));
    }
